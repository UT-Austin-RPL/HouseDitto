...
class Generator3D(object):
    ...
    def generate_mobile_points(self, data, return_stats=True):
        """Generates the output mesh.
        ...
        mobile_points_all, seg_probs_all = self.generate_from_latent_only_mobile_points(
            c, stats_dict=stats_dict, **kwargs
        )
        
        if return_stats:
            return mobile_points_all, seg_probs_all, c, stats_dict
        else:
            return mobile_points_all, seg_probs_all, c        
        
    def generate_from_latent_only_mobile_points(self, c=None, stats_dict={}, **kwargs):
        """Generates mesh from latent.
            Works for shapes normalized to a unit cube
        ...
        for index in (0, 1):
            ...
            if self.upsampling_steps == 0:
                ...
                mobile_points = self.eval_points_only_mobile_points(pointsf, c, index, **kwargs)
                mobile_points_list.append(mobile_points)
            else:
                mesh_extractor = MISE(
                    self.resolution0, self.upsampling_steps, threshold
                ...
                while points.shape[0] != 0:
                    ...
                    pointsf = torch.FloatTensor(pointsf).to(self.device)
                    # Evaluate model and update
                    mobile_points, seg_probs = self.eval_points_only_mobile_points(
                        pointsf, c, index, **kwargs
                    )                 
                    mobile_points_list.append(mobile_points)
                    seg_probs_list.append(seg_probs)
                    
                    values = np.random.rand(points.shape[0]) # fake value since occupancy is not used
                    values = values.astype(np.float64)
                    mesh_extractor.update(points, values)
                    points = mesh_extractor.query()
                    
        mobile_points_all = torch.cat(mobile_points_list, dim=1)
        seg_probs_all = torch.cat(seg_probs_list, dim=1)
        return mobile_points_all, seg_probs_all

    def eval_points_only_mobile_points(self, p, c, index, **kwargs):
        """Evaluates the occupancy values for the points.
        ...
        for pi in p_split:
            ...
            mobile_points.append(
                pi[
                    :,
                    (seg_prob > self.seg_threshold)[0],
                ]
            )
            seg_probs.append(seg_prob)
            
        mobile_points = torch.cat(mobile_points, dim=1)
        seg_probs = torch.cat(seg_probs, dim=1)
        return mobile_points, seg_probs