...
class PrismaticLoss(nn.Module):
    ...
    def forward(self, seg_label, pred_axis, pred_t, gt_axis, gt_t, debug=False):  # B*N
        ...
        loss += torch.nan_to_num(self.param_dict["p_ori_weight"] * loss_ori)
        # state loss
        if self.param_dict["p_use_state_loss"]:
            if self.param_dict["no_seg_mask"]:
                loss += torch.nan_to_num(F.l1_loss(pred_t, gt_t.unsqueeze(-1), reduction="none").mean(-1))
            else:
                loss += torch.nan_to_num(mask_mean(
        ...
        # B
        loss += torch.nan_to_num(self.param_dict["p_offset_weight"] * translation)
...
class RevoluteLoss(nn.Module):
    ...
    def forward(
        ...
        if self.param_dict["no_seg_mask"]:
            ...
            loss_ori = mask_mean(loss_ori, seg_label)
        loss += torch.nan_to_num(self.param_dict["r_ori_weight"] * loss_ori)
        ...
        loss += torch.nan_to_num(self.param_dict["r_p2l_ori_weight"] * loss_p2l_ori)
        ...
        loss += torch.nan_to_num(self.param_dict["r_p2l_dist_weight"] * loss_p2l_dist)
        ...
        if self.param_dict["r_use_state_loss"]:
            ...
            loss += torch.nan_to_num(self.param_dict["r_state_weight"] + loss_state)
        ...
        loss += torch.nan_to_num(self.param_dict["r_rot_weight"] * loss_rot)
        ...
        loss += torch.nan_to_num(self.param_dict["r_displacement_weight"] * displacement)
